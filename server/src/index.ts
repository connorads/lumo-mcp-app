...
const server = new McpServer(
  ...
  .registerWidget(
    ...
    {
      description:
        "Display a fill-in-the-blank exercise. Mark blanks as {{BLANK_ID}} in the prompt. CRITICAL: every blank must be deducible from the surrounding context alone — the learner should be able to work out the answer from what the sentence already says, without needing the hint. Bad: 'Je veux que tu {{verb}}' (could be ANY verb — eat, sleep, speak). Good: 'The subjunctive \"tu\" form of the verb \"parler\" is {{verb_form}}' (context constrains the answer). If the blank could be filled with multiple unrelated words, add more constraining text around it. Keep blank answers to 1-2 words where possible — shorter is fairer for recall. Always provide alternativeAnswers with 3-8 variants (abbreviations, short forms, synonyms, numeric equivalents) so learners aren't penalised for reasonable phrasings — but every alternative must be genuinely correct, not a grammatically wrong form. In your text response, tell the user to type answers and press Enter to check. Mention they can reveal answers if stuck after a couple of attempts.",
